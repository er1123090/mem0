...
class PersonaMemUploader:
    ...
    def _load_contexts(self, required_context_ids: Iterable[str]) -> Dict[str, Dict[str, object]]:
        ...
        Robust to JSONL shapes:
          (A) {"shared_context_id": "...", ...} / {"context_id": "...", ...} / {"id": "...", ...}
          (B) {"<context_id>": {... or [...]}}   # top-level single-key where key is the id
          (C) Nested wrappers (e.g., {"context": {"messages": [...]}})

        Skips blank lines, comment lines (#), and malformed JSON.
        Stops early if all `required_context_ids` are loaded.
        """
